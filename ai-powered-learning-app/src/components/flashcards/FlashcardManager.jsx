...
const FlashcardManager = ({documentId}) => {
  ...
  const fetchFlashingcardSets = async() => {
    setLoading(true)
    try {
      const response = await flashcardService.getFlashcardsForDocument(documentId)
      setFlashcardSets(response.data)
    } catch (error) {
      toast.error("Failded to fetch flashcard sets.")
      handleNotification("Failded to fetch flashcard sets.")
      console.log("Failed to fetch flashcard sets.")
    }finally{
      setLoading(false)
    }
  }
  ...
  const handleGenerateFlashcards = async () => {
    ...
    try {
      ...
      toast.success("Flascards generated successfully!")
      handleNotification("Flascards generated successfully!")
      fetchFlashingcardSets();
    } catch (error) {
  ...
  const handleConfirmDelete = async() => {
    ...
    try {
      ...
      handleNotification("Flashcard set deleted successfully!")
      setIsDeleteModalOpen(false)
      setSetToDelete(null)
      fetchFlashingcardSets()
    } catch (error) {
      toast.error(error.message || "Failde to delte flashcard set")
      handleNotification(error.message || "Failde to delte flashcard set")
    }finally{
      setDeleting(true)
    }
  }
          ...
          <div className='flex items-center gap-6'>
            <button
              ...
              <button onClick={handleNextCard} disabled={selectedSet.cards.length <= 1} 
                className='group flex items-center gap-2 px-5 h-11 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium text-sm rounded-xl transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-slate-100'
              >
                Next <ChevronRight className='w-4 h-4 group-hover:translate-x-0.5 transition-transform duration-200' strokeWidth={2.5}/>
              </button>
          </div>
        </div>
      </div>
        ...
        <div className='flex items-center justify-between'>
          <div>
            ...
            <p className='text-sm text-slate-500 mt-1'>
              {setFlashcardSets.length}{" "}
              {setFlashcardSets.length === 1 ? 'set' : "sets"} available
            </p>
          </div>
          <button
            onClick={handleGenerateFlashcards}
            disabled={generating}
            className='group inline-flex items-center gap-2 px-5 h-11 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:acitve:scale-100'
            >
              {generating? (
                <>
                  <div className='w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin'/>
              ...
              <div
                ...
                <div className='space-y-4'>
                  <div className='inline-flex items-center justify-center w-12 h-12 rounded-xl bg-linear-to-br from-emerald-100 to-teal-100'>
                    <Brain strokeWidth={2} className='w-6 h-6 text-emerald-600' />
                  </div>
    ...
    <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)}
      ...
      <div className='space-y-6'>
        ...
        <div className='flex items-center justify-end gap-3 pt-2'>
          <button type='button' onClick={() => setIsDeleteModalOpen(false)} disabled={deleting} className='px-5 h-11 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium text-sm rounded-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed'>
            Cancel
          </button>
